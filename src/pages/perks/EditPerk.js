...
const options = [{ value: "$12 W", label: "$12 W" }];
    ...
    return (
      <div className="perk-cont">
        ...
        <div className="right">
          ...
          <p className="">
            Use the guide to the left as a reference when creating your perk
            promos.
          </p>
          <div className="form">
            <Input
            ...
            />
            <CheckBox
              onChange={event => this.handleChange(event, "requires_qr_code")}
              style={{ marginBottom: "15px" }}
              label="Requires QR code"
              checked={perk.requires_qr_code}
            />
            <Input
              name="finePrint"
              onChange={event => this.handleChange(event, "fine_print")}
              label="Fine print"
              value={perk.fine_print}
            />
            <Input
              name="referral"
              onChange={event => this.handleChange(event, "required_referrals")}
              label="Required Referrals"
              value={perk.required_referrals}
            />
            <Input
              name="expire_date"
              onChange={event => this.handleChange(event, "expire_date")}
              label="Expire date"
              type="date"
              min="2019-03-05"
              max="2020-03-05"
              step="7"
              value={perk.expire_date}
            />
            <div className="btns">
              <Button
