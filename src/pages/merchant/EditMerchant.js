        ...
        return(
            <div className="edit-merchant-container">
                <UICard title="Edit merchant details">
                    <form>
                        <Input onChange={(event) => this.handleChange(event, 'name')} label="Name" value={merchant.name}/>
                        <Input onChange={(event) => this.handleChange(event, 'email')} label="Email" value={merchant.email}/>
                        <Input onChange={(event) => this.handleChange(event, 'contact_email')}  label="Contact Email" value={merchant.contact_email}/>
                        <Input onChange={(event) => this.handleChange(event, 'display_phone')}  label="Phone Number" value={merchant.display_phone}/>
                        <Input onChange={(event) => this.handleChange(event, 'description')} label="Description" value={merchant.description}/>
                        <Input onChange={(event) => this.handleChange(event, 'website')} label="Website" value={merchant.website}/>
                  
                        <label>Logo</label>
                        <img alt="" src={merchant.logo_url}/>
                        <div className="buttons">
                            <Button className="chooseImgBtn" text="Choose Image"></Button>
                            <Button className="uploadBtn" text="Upload"></Button>
                        </div>
                        ...
                        <Input onChange={(event) => this.handleChange(event, 'line1')} label="Address 1" value={merchant.address.line1}/>
                        <Input onChange={(event) => this.handleChange(event, 'line2')} label="Address 2" value={merchant.address.line2}/>
                        <Input onChange={(event) => this.handleChange(event, 'city')} label="City" value={merchant.address.city}/>
                        <Input onChange={(event) => this.handleChange(event, 'state')} label="State" value={merchant.address.state}/>
                        <Input onChange={(event) => this.handleChange(event, 'zip')} label="Zip" value={merchant.address.zip}/>
                        <Input onChange={(event) => this.handleChange(event, 'latitude')} label="Latitude" value={merchant.latitude}/>
                        <Input  onChange={(event) => this.handleChange(event, 'longitude')} label="Longitude" value={merchant.longitude}/>
                        ...
                        <div className="buttons">
                            <Button onClick={() => this.handleSubmit()}   themed={true} text="Save"/>
